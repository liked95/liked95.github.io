...
const todoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(todoApi.endpoints.updateTodos.matchFulfilled, (state, action) =>{
      let index = state.todos.findIndex(todo => todo.id === action.payload.id);
      state.todos[index] = action.payload
    })

    builder.addMatcher(todoApi.endpoints.deleteTodos.matchFulfilled, (state, action) =>{
